...
```{r, echo=T}
origin_sc <- scale(reduced_tr[,-53])            ## scaling madatory before SVD. 
svd1 <-svd(origin_sc)
sum(svd1$d[1:30]^2)/sum(svd1$d^2)               ## 30 first components over 52 --> > 97% variance

tr_newbase_sc <- t(svd1$v) %*% t(origin_sc)     ## gives the transposed matrix of the components of the 1000 instances in the new base   (V )
newbase_sc <- t(tr_newbase_sc)                  ## gives the actual matrix of the components of the 1000 instances in the new base, which orthogonal vectors are directed to the principal variance directions
round(colSums(newbase_sc^2))                    ## actually shows the decrease of variance explained: the first columns of the training set in the new base (principal componets base) vary in a larger way

```{r, echo=T,results='hide'}
approx1_30 <- as.data.frame(newbase_sc[,1:30])
...
modFit
```{r, echo=T}
modFit$finalModel               ## confusion matrix and oob error rate 21% !!! No. of variables tried at each split: 2
```
...
answers <- predict(modFit_R,usefull_test)
